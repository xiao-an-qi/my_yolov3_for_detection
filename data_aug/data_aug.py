...
class RandomVertialFlip(object):
    ...
    def __call__(self, img, bboxes):
            img_center = np.array(img.shape[:2])[::-1]/2       # 图像中心
            img_center = np.hstack((img_center, img_center))   # 复制一次，宽度为4
            if random.random() < self.p:
                img = img[::-1, :, :]     # 垂直镜像
                bboxes[:, [1, 3]] = bboxes[:, [1, 3]] +  2*(img_center[[1, 3]] - bboxes[:, [1, 3]])  # 沿图像中心线水平翻转

                box_h = abs(bboxes[:, 3] - bboxes[:, 1])
...
class RandomHorizontalFlip(object):
    ...
    def __call__(self, img, bboxes):
            ...
            if random.random() < self.p:
                img = img[:, ::-1, :]     # 水平镜像
                bboxes[:, [0, 2]] = bboxes[:, [0, 2]] + 2*(img_center[[0, 2]] - bboxes[:, [0, 2]])  # 沿图像中心线水平翻转

                box_w = abs(bboxes[:, 0] - bboxes[:, 2])    # 初始框的宽度
...
class RandomRotate(object):
    ...
    def __call__(self, img, bboxes):
        ...
        new_bbox[:,:4] = np.true_divide(new_bbox[:,:4], [scale_factor_x, scale_factor_y, scale_factor_x, scale_factor_y], out=None)
        bboxes  = new_bbox
...
class RandomShear(object):
    ...
    def __call__(self, img, bboxes):
        ...
        bboxes[:,:4] = np.true_divide(bboxes[:,:4], [scale_factor_x, 1, scale_factor_x, 1], out=None)
        return img, bboxes
...
class RandomBlur(object):
    """Randomly blur and add noises to  the Image with the probability *p*
    ...
    def __call__(self, img, bboxes):
            if random.random() < self.p:
                img = cv2.GaussianBlur(img,(9,9),0)     # 高斯滤波
            if random.random() < self.p:
                noise = np.random.normal(0, 0.01 ** 0.5, img.shape)
                img = img + noise
            return np.uint8(img), bboxes
