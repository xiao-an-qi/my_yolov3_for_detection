...
def _main():
    ...
    if is_tiny_version:
        ...
        model = create_model(input_shape, anchors, num_classes,freeze_body=2, weights_path='model_data/yolo_save.h5')     # make sure you know what you freeze
    ...
    if True:
        ...
        model.save_weights(log_dir + 'trained_weights_60.h5')
        model.save(log_dir + 'yolo_save60.h5')
...
def get_anchors(anchors_path):
    ...
    with open(anchors_path) as f:
        anchors = f.readline()     # only one line
...
def create_tiny_model(input_shape, anchors, num_classes, load_pretrained=True, freeze_body=2, weights_path='model_data/tiny_yolo_weights.h5'):
    ...
    K.clear_session()   # get a new session
    image_input = Input(shape=(None, None, 3))    # 输入为3通道
    h, w = input_shape
    num_anchors = len(anchors)
